...
## UC Davis ----
UCD = read.csv("./uncleaned_data/ucd_course_catalog.csv", check.names = FALSE)
...
#write.csv(UCD_clean, "./cleaned_data/ucd_course_catalog_CLEAN.csv", row.names = FALSE, 
       #   fileEncoding = "UTF-8", na = "")

## UCLA ----
UCLA = read.csv("./uncleaned_data/UCLA.csv")
...
#write.csv(UCLA_clean, "./cleaned_data/ucla_CLEAN.csv", row.names = FALSE, 
#          fileEncoding = "UTF-8", na = "")

## UC Irvine ----
UCI = read.csv("./uncleaned_data/uci_courses_catalog.csv", check.names = FALSE)
...
#write.csv(UCI_clean, "./cleaned_data/uci_courses_catalog_CLEAN.csv", row.names = FALSE, 
        #  fileEncoding = "UTF-8", na = "")

## UC Santa Cruz ----
UCSC = read.csv("./uncleaned_data/ucsc_final_cleaned.csv", check.names = FALSE)
...
#write.csv(UCSC_clean, "./cleaned_data/ucsc_courses_catalog_CLEAN.csv", row.names = FALSE, 
#          fileEncoding = "UTF-8", na = "")

## UC San Diego
UCSD = read.csv("./uncleaned_data/ucsd_courses.csv", check.names = FALSE)

UCSD_clean = UCSD %>% 
  filter(!(Subject == "Biomedical Sciences" & `Subject Code` == "AASM")) %>% 
  mutate(Cross_Listing = str_extract(Description, "\\(?Cross-listed with[^\\)\\.]+\\)?\\.?"),
         Cross_Listing = gsub("\\(?Cross-listed with |\\.|\\)", "", Cross_Listing),
         Description = gsub("\\(?Cross-listed with[^\\)\\.]+\\)?\\.?", "", Description),
         
         Same_As = str_extract(Description, "\\(?Same as[^\\)\\.]+\\)?\\.?"),
         Same_As = gsub("\\(?Same as |\\.|\\)", "", Same_As),
         Description = gsub("\\(?Same as[^\\)\\.]+\\)?\\.?", "", Description),
         
         students_may = str_extract(Description, "Students may[^\\.]+\\."),
         Description = gsub("Students may[^\\.]+\\.", "", Description),
         ...
         Description = gsub("\\(Formerly,? [^\\)]+\\)|Formerly [^\\)]+\\)?|\\([^)]*formerly[^)]*\\)?", 
                            "", Description),
         pass = str_extract(Description, "\\(?Pass/No[^\\.]+\\.|\\(?P/NP grades only\\.\\)?"),
         Description = gsub("\\(?Pass/No[^\\.]+\\.|\\(?P/NP grades only\\.\\)?", "", Description),
         
         renumbered = str_extract(Description, "Renumbered[^\\.]+\\.|[A-Z]{2,5}\\s*\\d+[A-Z]? is renumbered from [A-Z]{2,5}\\s*\\d+[A-Z]?\\."),
         Description = gsub("Renumbered[^\\.]+\\.|[A-Z]{2,5}\\s*\\d+[A-Z]? is renumbered from [A-Z]{2,5}\\s*\\d+[A-Z]?\\.", "", Description),
         
         Cross_Listing = coalesce(Cross_Listing, Same_As),
         Cross_Listing = gsub(", and ", " and ", Cross_Listing),
         Cross_Listing = gsub(" and |\\/", ", ", Cross_Listing),
         Cross_Listing = na_if(Cross_Listing, "the Brain, Wildlife Conservation"),
    
         Cross_Listing = case_when(
           str_detect(Cross_Listing, regex("^PSYC\\s*133R", ignore_case = TRUE)) ~ "PSYC 133R",
           str_detect(Cross_Listing, regex("^PSYCH?\\s*133(?!R)", ignore_case = TRUE)) ~ "PSYC 133",
           TRUE ~ Cross_Listing),
         
         coscheduled = str_extract(Description, "\\(?((May be )?Coscheduled with|May be coscheduled with) [A-Z]{2,5}\\s*\\d+[A-Z]?\\.\\)?"),
         Description = gsub("\\(?((May be )?Coscheduled with|May be coscheduled with) [A-Z]{2,5}\\s*\\d+[A-Z]?\\.\\)?", "", Description),
         
         Description = gsub("^\\) ", "", Description)) %>% 
  select(-`Cross Listing`, -Same_As)

#write.csv(UCSD_clean, "./cleaned_data/ucsd_courses_catalog_CLEAN.csv", row.names = FALSE, 
#          fileEncoding = "UTF-8", na = "")
...
#write.csv(combined, "./cleaned_data/combined_CLEAN.csv", row.names = FALSE, 
#          fileEncoding = "UTF-8", na = "")
...
write.csv(canonical_df, "./cleaned_data/canonical_CLEAN.csv", row.names = FALSE, 
          fileEncoding = "UTF-8", na = "")
write.csv(clusters, "./cleaned_data/course_clusters.csv", row.names = FALSE, 
          fileEncoding = "UTF-8", na = "")

## evaluation set
evaluation = read.csv("./uncleaned_data/Evaluation_Set.csv", check.names = FALSE)
...
write.csv(evaluation_short, "./cleaned_data/evaluation.csv", row.names = FALSE, 
          fileEncoding = "UTF-8", na = "")
